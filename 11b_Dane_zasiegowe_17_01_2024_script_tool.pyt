...
# getParameterInfo #############################################################################################################################################################################################################################################
    def getParameterInfo(self):
        ...
        #cos z tym parametrem 11 jest nie tak, moze jest już innym numerem
...
#         param6_9.value = r'D:\ArcGIS\3_Dane_Zasiegowe_LTE_4G_UMTS_3G_GSM_2G\Dane_zasiegowe_nr_2_styczen_2024\2024_02_09_MV_Dane_zasiegowe_new_new.gdb'
        ...
        #param13.value = r'D:\ArcGIS\2_Dane_Zasiegowe_5G_DSS\5G_DSS_nr_1_grudzien_2023\Dane\Dane_z_22_01_2024\cov_dss_20240101_poprawa_4'
...
#         param22.value = False
        ...
        params = [param0, param_lcell_7_1, param_wcell_8_2, param_cell_9_3, param1_4, param2_5, param3_6, param4_7, param5_8, param6_9, param_przeciazenia_10, param11, param12, param13, param14, param15, param16, param17, param18, param19, param20, param21, param22, param23, param24]
...
#                 arcpy.AddMessage(f'files_5G_DSS: {files}')
            ...
            for sciezka in lista_1_5G:
                ...
                arcpy.AddMessage(f'-------------------------------')
                arcpy.AddMessage(f'-------------------------------1440')
                arcpy.AddMessage(f'Warstwa: {wynik_7}')
#                 arcpy.AddMessage(f'output_info: {output_info}')
#                 arcpy.AddMessage(f'output_feature_class_info_name: {output_feature_class_info_name}')
                arcpy.AddMessage(f'-------------------------------')
                arcpy.AddMessage(f'-------------------------------1445')
...
# POPRAWKI #############################################################################################################################################################################################################################################
        if param_1212:
            ...
            path1 = arcpy.env.workspace = parameters[13].valueAsText
            # List all TIFF rasters with the pattern *_DSS*.tif
            raster1 = arcpy.ListRasters("*_DSS*.tif", "TIF")
            path2 = arcpy.env.workspace = parameters[9].valueAsText
            pol1 = arcpy.ListFeatureClasses("*M_DSS*")

            # Perform the Extract by Mask operation
            out_raster = arcpy.sa.ExtractByMask(fr"{path1}\\{raster1[0]}", fr'{path2}\\{pol1[0]}')
            outExtractByMask = out_raster.save(fr'{path1}\\DSS.tif')
            # Save the output raster

            #lista_warstw_all.append(outExtractByMask)
            arcpy.AddMessage(f'Gotowy raster: {outExtractByMask}---------------------- KONIEC - CZESCI VI - Rastr DSS docięty do maski ----------------------')
            ...
            lista_warstw_all = arcpy.ListRasters('*', "TIF")
            arcpy.AddMessage(f'-------------------------------')
            arcpy.AddMessage(f'-------------------------------1544')
            arcpy.AddMessage('Rastery, na których zostaną wykonane piramidy - w wyniku nasze rastry wejsciowe będą już z piramidami i w układzie 3857: ')
            arcpy.AddMessage(lista_warstw_all)
            arcpy.AddMessage(f'-------------------------------')
            arcpy.AddMessage(f'-------------------------------')
            # sprawdzić za ostanim plusem dla UMTS DLA CAŁOŚĆI
            lista_warstw = arcpy.ListRasters("*LTE*" + "*_5class_*" + "*.tif", "TIF") + arcpy.ListRasters("CA_OPL_*" + "*.tif", "TIF") + arcpy.ListRasters("*_UMTS*" + "_5class_*" + "*.tif", "TIF") + arcpy.ListRasters("*_GSM*" + "_5class_*" + "*.tif", "TIF") + arcpy.ListRasters("*_UMTS*" + "*.tif", "TIF") + arcpy.ListRasters("*DSS*" + "*.tif", "TIF") + arcpy.ListRasters("*_CBAND*" + "*.tif", "TIF")
            ...
            for warstwa_rastrowa in lista_warstw_all:
                ...
                #arcpy.AddMessage(f'---------------TEST----------------')
                #arcpy.AddMessage(fr'{aa}\{warstwa_rastrowa}')
                #arcpy.AddMessage(f'--------{aa}\{warstwa_rastrowa}----- Linia 1587------------------')
                #arcpy.AddMessage(f'------------- Linia 1588------------------')
                #arcpy.env.overwriteOutput = True
                if "PUWG92_" not in warstwa_rastrowa:
                    arcpy.env.overwriteOutput = True
                    path_PUWG_92 = parameters[24].valueAsText
                    arcpy.management.ProjectRaster(fr'{aa}\{warstwa_rastrowa}', fr'{path_PUWG_92}\{warstwa_rastrowa}', e2180)

                    arcpy.AddMessage(f'PUWG92_{warstwa_rastrowa}---------------------- Rastr zamieniony na PUWG92 ----------------------')
            ...
            # lista_warstw = arcpy.ListFeatureClasses('*GES_*') + arcpy.ListFeatureClasses('*OT_*')
            #lista_warstw_all_mv = arcpy.ListFeatureClasses('*')
            lista_warstw_all_mv = arcpy.ListFeatureClasses("*M_LTE*") + arcpy.ListFeatureClasses("CA_OPL_*") + arcpy.ListFeatureClasses("*M_UMTS*") + arcpy.ListFeatureClasses("*M_GSM*") + arcpy.ListFeatureClasses("*M_CBAND*")+ arcpy.ListFeatureClasses("*M_DSS*")
            arcpy.AddMessage(f'-------------------------------')
            ...
            arcpy.AddMessage(f'---------------------- KONIEC - CZESCI VIII - poprawki DOM C-band i DSS do geobazy 3857 ----------------------')
