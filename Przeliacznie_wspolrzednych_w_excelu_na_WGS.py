...
def dms_to_decimal(dms_str):
    ...
    # Usuń litery kierunkowe (N, S, E, W) i białe znaki
    dms_str = re.sub(r'[NSEW]', '', dms_str).strip()
    # Zastąp przecinki kropkami
    ...
    try:
        ...
        raise ValueError(f"Nieprawidłowy format współrzędnych: {dms_str}")
...
def przelicz_wspolrzedne_csv(input_file, output_file):
    ...
    for row in data[1:]:
        try:
            if row[latitude_index].strip() and row[longitude_index].strip():
                row[latitude_index] = dms_to_decimal(row[latitude_index])
                row[longitude_index] = dms_to_decimal(row[longitude_index])
        except ValueError as e:
            messagebox.showerror("Błąd konwersji", f"Błąd konwersji w wierszu: {row}\n{e}")
            return
